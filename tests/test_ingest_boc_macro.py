...
class TestFetchBoc:
    ...
    def test_series_values_extracted(self, mock_get, mock_makedirs, mock_to_csv):
        obs = [{'d': '2024-01-01', 'CAOVERAGE': {'v': '4.5'}, 'FXCADUSD': {'v': '0.73'}}]
        mock_get.return_value = self._mock_response(obs)
        df = boc.fetch_boc()
    ...
    @patch('ingest_boc_macro.os.makedirs')
    @patch('ingest_boc_macro.requests.get')
    def test_api_called_with_correct_url_and_params(self, mock_get, mock_makedirs, mock_to_csv):
        mock_get.return_value = self._mock_response([])
        boc.fetch_boc()
    ...
    @patch('ingest_boc_macro.os.makedirs')
    @patch('ingest_boc_macro.requests.get')
    def test_empty_observations_returns_empty_df(self, mock_get, mock_makedirs, mock_to_csv):
        mock_get.return_value = self._mock_response([])
        df = boc.fetch_boc()
        assert len(df) == 0
    ...
    @patch('ingest_boc_macro.os.makedirs')
    @patch('ingest_boc_macro.requests.get')
    def test_multiple_observations_all_rows_present(self, mock_get, mock_makedirs, mock_to_csv):
        obs = [
    ...
    @patch('ingest_boc_macro.os.makedirs')
    @patch('ingest_boc_macro.requests.get')
    def test_missing_observations_key_handled(self, mock_get, mock_makedirs, mock_to_csv):
...
class TestMainBlock:
    def test_main_calls_fetch_and_load(self):
        ...
        try:
            ...
            df = boc.fetch_boc()
            try:
                boc.load_csv_chunked('data/boc_macro.csv', 'BOC_MACRO', boc.CONFIG, chunksize=50000)
