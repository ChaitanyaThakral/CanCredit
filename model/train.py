...
def train_and_evaluate(X, y):
    ...
    with mlflow.start_run(run_name="logistic_regression_baseline"):
        ...
        print(
            f"LR    │ AUC: {lr_scores.mean():.4f} ± {lr_scores.std():.4f} "
    ...
    with mlflow.start_run(run_name="xgboost_tuned"):
        ...
        print(
            f"XGB   │ AUC: {xgb_cv.mean():.4f} ± {xgb_cv.std():.4f} "
        ...
        mlflow.sklearn.log_model(
            xgb, "xgb_credit_model", registered_model_name="cancredit_xgb_v1"
        )
        fi = dict(zip(FEATURES, xgb.feature_importances_.tolist()))
        fi_path = "/tmp/feature_importances.json"
        with open(fi_path, "w") as f:
            json.dump(
                dict(sorted(fi.items(), key=lambda x: x[1], reverse=True)), f, indent=2
            )
...
def evaluate_and_plot(xgb, X, y):
    ...
    shap.summary_plot(
        shap_vals, X_sample, feature_names=FEATURES, plot_type="bar", show=False
    )
