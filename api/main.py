...
if not MODEL_PATH.exists():
    import warnings
    warnings.warn(f"Model not found at {MODEL_PATH}. Creating a dummy model for testing.")
    from xgboost import XGBClassifier
    import shap
    
    # Create synthetic data to train a dummy model
    dummy_X = pd.DataFrame(np.random.rand(10, 18), columns=[
...
@app.get("/health")
def health():
    return {"status": "ok", "model": "xgb_credit_v1",
