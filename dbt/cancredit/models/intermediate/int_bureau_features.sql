...
SELECT
    ...
    SUM(CASE WHEN b.credit_status = 'Active'  THEN 1 ELSE 0 END)    AS bureau_active_credits,
    SUM(CASE WHEN b.credit_status = 'Closed'  THEN 1 ELSE 0 END)    AS bureau_closed_credits,
    ...
    COALESCE(MAX(
        CASE WHEN bb.status IN ('1','2','3','4','5')
             THEN bb.status::INT ELSE 0 END), 0)                     AS bureau_worst_delinquency,
    -- Proportion of months in delinquency
    COALESCE(ROUND(AVG(
        CASE WHEN bb.status NOT IN ('0','X','C') THEN 1.0
             ELSE 0.0 END), 4), 0)                                   AS bureau_delinquency_rate
FROM bureau b
LEFT JOIN bureau_bal bb ON b.bureau_id = bb.bureau_id
